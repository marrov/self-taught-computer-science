...
-- Find all caller phone number which made a call for less than a minute during the day of the crime
SELECT caller FROM phone_calls WHERE duration BETWEEN 0 AND 59 AND month = 7 AND day = 28;
-- Narrow down the list even more with the names of the people that made the less-than-a-minute call - Found the thief!
SELECT name FROM people WHERE license_plate IN (SELECT license_plate FROM courthouse_security_logs WHERE month = 7 AND day = 28 AND hour = 10 AND minute >= 15 AND minute <= 25) INTERSECT SELECT name FROM people WHERE id IN (SELECT person_id FROM bank_accounts WHERE account_number IN (SELECT account_number FROM atm_transactions WHERE month = 7 AND day = 28 AND atm_location = "Fifer Street" AND transaction_type = "withdraw")) INTERSECT SELECT name FROM people WHERE passport_number IN (SELECT passport_number FROM passengers WHERE flight_id = (SELECT id FROM (SELECT id, min(hour) FROM flights WHERE origin_airport_id = (SELECT id FROM airports WHERE city = "Fiftyville") AND month = 7 AND day = 29))) INTERSECT SELECT name FROM people WHERE phone_number IN (SELECT caller FROM phone_calls WHERE duration BETWEEN 0 AND 59 AND month = 7 AND day = 28);
-- Find where thief travelled to
SELECT city FROM airports WHERE id = (SELECT destination_airport_id FROM (SELECT destination_airport_id, min(hour) FROM flights WHERE origin_airport_id  = (SELECT id FROM airports WHERE city = "Fiftyville") AND month = 7 AND day = 29));
-- Find the thief's accomplice
SELECT name FROM people WHERE phone_number = (SELECT receiver FROM phone_calls WHERE duration BETWEEN 0 AND 59 AND month = 7 AND day = 28 AND caller = (SELECT phone_number FROM people WHERE name = "Ernest"));